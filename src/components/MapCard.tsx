...
export default function MapCard() {
  ...
  const normalizeName = (name: string) => {
    ...
    if (["PAPUATENGAH", "PAPUAPEGUNUNGAN", "PAPUASELATAN"].includes(n))
      return "PAPUA";

    return n;
  };

  // Fetch data provinsi dari Laravel API
  useEffect(() => {
    async function fetchProvinceStats() {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/statistics/provinces`,
        );
        if (!res.ok) throw new Error("Failed to fetch province stats");
        const data = await res.json();

        // Convert ke format { PROVINCENAME: count }
        const s: Record<string, number> = {};
        data.forEach((item: { province: string; total: number }) => {
          const cleanName = normalizeName(item.province);
          if (cleanName) {
            s[cleanName] = (s[cleanName] || 0) + item.total;
          }
        });
        setStats(s);
      } catch (error) {
  ...
  // Render GeoJSON layer — re-render setiap kali stats berubah
  useEffect(() => {
  ...
  }, [geoData, stats]); // ← re-render saat stats dari API sudah loaded
    ...
    <div className="w-full bg-white rounded-[8px] border border-[#D0B4B4] overflow-hidden flex flex-col shadow-sm h-full font-poppins">
      ...
      <div className="flex-grow relative m-4 mt-2 bg-[#F0F7FF] rounded-lg border border-blue-50 overflow-hidden min-h-[450px]">
        ...
        <div className="absolute top-4 right-4 z-[10] bg-white/95 backdrop-blur-sm p-3 rounded-lg shadow-sm border border-gray-100 min-w-[120px]">
          <h4 className="text-[9px] font-bold text-gray-500 uppercase mb-2">
            {t.numconTitle}
          </h4>
          <div className="space-y-1.5">
            <LegendItem color="#E91E63" label={`> 80 ${t.cons}`} />
            <LegendItem color="#FF5722" label={`30-80 ${t.cons}`} />
            <LegendItem color="#FF9800" label={`10-30 ${t.cons}`} />
            <LegendItem color="#FFEB3B" label={`< 10 ${t.cons}`} />
          </div>
        </div>
      </div>
    </div>
