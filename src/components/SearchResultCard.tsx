...
const SearchResultCard: React.FC<SearchResultCardProps> = ({
  ...
  const photoUrl = imageUrl || defaultImageUrl;

  const statusClass =
    status === "aktif" ? "bg-green-500 text-white" : "bg-red-500 text-white";

  const translatedStatus = status === "aktif" ? t.active : t.inactive;
  ...
  return (
    ...
    >
      <div className="flex-shrink-0 relative w-44 h-full mr-6">
        ...
        <span
          className={`absolute top-0 right-0 px-3 py-1 text-xs font-semibold uppercase rounded-full ${statusClass} transform translate-x-1/4 -translate-y-1/4`}
        >
          {translatedStatus}
        </span>
      </div>
